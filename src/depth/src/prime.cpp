...
ros::init(argc, argv, "prime_node");

cout << "Ready to stream" << endl;
ros::NodeHandle nh;
ros::Subscriber sub = nh.subscribe("camera/rgb/image_color",MY_ROS_QUEUE_SIZE, depth_node_callback);

cv::namedWindow("prime_node");
ros::spin();
cv::destroyWindow("prime_node");
